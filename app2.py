...
if data.empty:
    ...
    # Initialiser l'état pour l'échelle logarithmique
    if "log_scale" not in st.session_state:
        st.session_state["log_scale"] = False

    # Bouton pour basculer l'affichage en échelle logarithmique
    ...
    # On active les marqueurs pour faciliter la détection des clics
    st.subheader(f"Cours de {selected_ticker}")
    fig = go.Figure()
    ...
    click_data = plotly_events(fig, hover_event=True, override_height=500)
    print(click_data)
    # Si au moins deux clics ont été effectués, on calcule le pourcentage d'évolution
    if st.session_state.old_click_data:
